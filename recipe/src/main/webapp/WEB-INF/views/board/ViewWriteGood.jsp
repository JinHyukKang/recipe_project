		...
		<div class="col-md-12">
			...
			<div class="form-group row mt-3 mx-3 mb-2" style="border: 1px solid; border-radius: 10px;background-color: lightgray;">
				...
				<c:if test="${user_id ne null}">
					<form method="post" action="/board/commentWriteGood.do">
				...
				<!-- 댓글 출력 -->
					...
					<c:forEach items="${commentView}" var="commentView">
							...
							<!-- 댓글 수정 -->
						    <form method="post" action="/board/commentUpdateGood.do" id="commentForm-${commentView.comment_num}">
...
function deleteComment(commentNum, recipeNum,event) {
    ...
    if (confirm("댓글을 삭제하시겠습니까?")) {
        $.ajax({
            url: '/board/commentDelGood.do',
            type: 'post',
            data: { comment_num: commentNum, recipe_num: recipeNum },
            success: function(response) {
                alert("댓글이 삭제되었습니다.");
                location.href = '/board/ViewWriteGood?recipe_num=' + recipeNum;
            },
...
function cancelUpdate(event, recipeNum) {
    ...
    window.location.href = '/board/ViewWriteGood?recipe_num=' + recipeNum;
    return false;
}
</script>
