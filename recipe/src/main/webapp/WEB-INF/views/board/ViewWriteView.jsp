		...
		<div class="col-md-12">
			...
			<div class="form-group row mt-3 mx-3 mb-2" style="border: 1px solid; border-radius: 10px;background-color: lightgray;">
				...
				<c:if test="${user_id ne null}">
					<form method="post" action="/board/commentWriteView.do">
				...
				<!-- 댓글 출력 -->
					...
					<c:forEach items="${commentView}" var="commentView">
							...
							<!-- 댓글 수정 -->
						    <form method="post" action="/board/commentUpdateView.do" id="commentForm-${commentView.comment_num}">
			        ...
			        <div style="border: 1px solid; border-radius: 10px; width: 400px; margin-left:15px; margin-right:220px;">
			            <div style="height:30px;">
			                <p id="prePage" style="margin-left:10px;">
			                    ...
			                    style="white-space: nowrap;overflow: hidden; text-overflow: ellipsis; max-width: 80%; display: inline-block; color: black; text-decoration: none;">
...
function deleteComment(commentNum, recipeNum,event) {
    ...
    if (confirm("댓글을 삭제하시겠습니까?")) {
        $.ajax({
            url: '/board/commentDelView.do',
            type: 'post',
            data: { comment_num: commentNum, recipe_num: recipeNum },
            success: function(response) {
                alert("댓글이 삭제되었습니다.");
                location.href = '/board/ViewWriteView?recipe_num=' + recipeNum;
            },
...
function cancelUpdate(event, recipeNum) {
    ...
    window.location.href = '/board/ViewWriteView?recipe_num=' + recipeNum;
    return false;
}
</script>
